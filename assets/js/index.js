    ...
    if (sublineEl && explainerEl) {
        if (!generatedAt && numericRows === 0) {
            explainerEl.innerHTML =
                'No snapshot found. Run <code class="font-mono text-[11px]">just fetch</code> then <code class="font-mono text-[11px]">just index</code> (or the equivalent <code class="font-mono text-[11px]">uv run python -m scripts.…</code>), or wait for CI.';
